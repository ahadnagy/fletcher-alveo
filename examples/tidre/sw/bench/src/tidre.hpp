...
class Tidre {
 ...
 public:
  ...
  static fletcher::Status Make(
    ...
    TIDRE_CHECK_STATUS(tidre->Tweak(numPipelineBeats, numKernelsToUse));
    *out = tidre;
    return fletcher::Status::OK();
  }
  ...
  fletcher::Status RunRaw(
    ...
    for (size_t beat = 0; beat <= numPipelineBeats; beat++) {
      ...
      if (beat < numPipelineBeats) {
        ...
        bool alloced;
        da_t offs_device_addr;

        TIDRE_CHECK_STATUS(platform->PrepareHostBuffer((uint8_t*)offs_part_src, &offs_device_addr, offs_part_size, &alloced));
